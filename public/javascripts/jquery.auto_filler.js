...
$.fn.setStore = function ( options) {
	...
	{
			$.log("== combine cookie_value: "+ key + " = [" + value +"]");
			cookie_value += key +"="+ value +";";
	});

	// save cookie
	$.log("====================================");
	$.log("====== save to store: "+ af_cookie_name());
	$.log("=== store content: "+ cookie_value);
...
function setupAutofiller()
{
	...
	$('#auto_filler').append("<div><input type='text' size='10' class='af_field' id='af_version_field' name='af_version_field' value='urlVersion'/></div>");
...
function af_cookie_name () {
	...
	try
	{
		if(urlVersion!=null && urlVersion!="" && urlVersion!="urlVersion")
		{
